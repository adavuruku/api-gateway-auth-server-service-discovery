...
import com.example.apigateway.model.Authorities;
import com.example.apigateway.model.ConnValidationResponse;
import com.example.apigateway.utils.AppConstants;
...
public class AuthenticationPrefilter extends AbstractGatewayFilterFactory<AuthenticationPrefilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            String bearerToken = request.getHeaders().getFirst(AppConstants.HEADER);
            log.info("Bearer Token: "+ bearerToken);
            log.info("isSecured.test(request) : {} {}",isExcludeUrls.test(request), request.getURI().getPath());

            if(isExcludeUrls.test(request)) {
                return webClientBuilder.build().get()
                        .uri("lb://authentication-service/api/v1/validateToken")
                        .header(AppConstants.HEADER, bearerToken)
                        ...
                        .map(response -> {
                            ...
                            log.info("response res {} {} {}", response.getUsername(), response.getAuthorities(),response.getToken() );
...
//                            AuthorizationFilter.AUTH_FAILED_CODE
    ...
    public Predicate<ServerHttpRequest> isExcludeUrls = request -> excludedUrls.stream().noneMatch(uri -> {
...
//            ExceptionResponseModel data = new ExceptionResponseModel(errCode, err, errDetails, null, new Date());
            byte[] byteData = objectMapper.writeValueAsBytes(errCode);
